...
{#each phases as phase, i}
  ...
  >
    ...
    <text x={zlX} y={phase.wireY + 1} fill={phase.color} font-size="9" text-anchor="middle" dominant-baseline="middle" font-family="var(--font-mono)">Zl</text>
  </InteractiveElement>
  {#if circuitState.lineImpedances[i]}
    <text x={zlX} y={phase.wireY + 17} fill={phase.color} font-size="8" text-anchor="middle" font-family="var(--font-mono)" opacity={isDimmed ? 0.2 : 0.6}>{fmtVal(circuitState.lineImpedances[i])}</text>
  {/if}
  ...
  <text x={arrowX + 12} y={phase.wireY - 22} fill={phase.color} font-size="12" text-anchor="middle" font-family="var(--font-mono)" font-weight="500">{currentLabels[i]}</text>
{/each}
