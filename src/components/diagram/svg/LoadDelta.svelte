...
{#each loads as load, i}
  ...
  >
    ...
    <text x={load.x} y={load.y + 1} fill={load.color} font-size="8" text-anchor="middle" dominant-baseline="middle" font-family="var(--font-mono)">{load.label}</text>
  </InteractiveElement>

  <!-- Value label -->
  {#if circuitState.loadImpedances[i]}
    {#if i === 0}
      <text x={load.x - 22} y={load.y + 2} fill={load.color} font-size="7" text-anchor="end" font-family="var(--font-mono)" opacity={isDimmed ? 0.2 : 0.6}>{fmtVal(circuitState.loadImpedances[i])}</text>
    {:else if i === 1}
      <text x={load.x} y={load.y + 18} fill={load.color} font-size="7" text-anchor="middle" font-family="var(--font-mono)" opacity={isDimmed ? 0.2 : 0.6}>{fmtVal(circuitState.loadImpedances[i])}</text>
    {:else}
      <text x={load.x + 22} y={load.y + 2} fill={load.color} font-size="7" text-anchor="start" font-family="var(--font-mono)" opacity={isDimmed ? 0.2 : 0.6}>{fmtVal(circuitState.loadImpedances[i])}</text>
    {/if}
  {/if}
{/each}
...
<text x={wireInX + 2} y={wireYs[0] - 10} fill="var(--phase-a)" font-size="12" text-anchor="start" font-weight="600">A</text>

<line x1={wireInX} y1={wireYs[1]} x2={tri.B.x} y2={tri.B.y} stroke="var(--svg-wire)" stroke-width="1.5" />
<text x={wireInX + 2} y={wireYs[1] - 10} fill="var(--phase-b)" font-size="12" text-anchor="start" font-weight="600">B</text>
...
<text x={wireInX + 2} y={wireYs[2] - 10} fill="var(--phase-c)" font-size="12" text-anchor="start" font-weight="600">C</text>

<!-- Vertex labels -->
<text x={tri.A.x + 6} y={tri.A.y - 6} fill="var(--text-muted)" font-size="10" font-weight="500">A</text>
<text x={tri.B.x - 10} y={tri.B.y - 6} fill="var(--text-muted)" font-size="10" font-weight="500">B</text>
<text x={tri.C.x + 6} y={tri.C.y - 6} fill="var(--text-muted)" font-size="10" font-weight="500">C</text>
