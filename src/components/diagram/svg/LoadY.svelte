...
<text x={neutralX + 10} y={neutralY + 5} fill="var(--text-muted)" font-size="10">N</text>

{#each phases as phase, i}
  <!-- Wire from line to load -->
  <line x1={500} y1={phase.wireY} x2={loadX - 20} y2={phase.wireY} stroke="var(--svg-wire)" stroke-width="1.5" />

  <!-- Terminal label -->
  <text x={502} y={phase.wireY - 10} fill={phase.color} font-size="10" text-anchor="start" font-weight="600">
    {['A', 'B', 'C'][i]}
  </text>
  ...
  {#if circuitState.loadImpedances[i]}
    <text x={loadX} y={phase.wireY + 16} fill={phase.color} font-size="6" text-anchor="middle" font-family="var(--font-mono)" opacity="0.6">{fmtVal(circuitState.loadImpedances[i])}</text>
  {/if}
